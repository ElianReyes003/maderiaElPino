...
use App\Models\CambiosArticulos;
use Carbon\Carbon;
...
class articulo_controller extends Controller
{
    public function todosLosArticulos(Request $request) {
        ...
        $datosArticulos = Articulo::join('articulotipoventa', 'articulotipoventa.fkArticulo', '=', 'articulo.pkArticulo')    
            ...
            ->where('articulotipoventa.fkTipoVenta', '=', 4) ->where('articulo.estatus', '!=', 3)
            ->get();
    ...
    public function articuloDetalle($pkArticulo, $vista = "articuloDetalle") {
        ...
        $datosArticulosGenerales = Articulo::join('articulotipoventa', 'articulotipoventa.fkArticulo', '=', 'articulo.pkArticulo')
        ->join('categoriaarticulo', 'articulo.fkCategoriaArticulo', '=', 'categoriaarticulo.pkCategoriaArticulo')
        ->select('categoriaarticulo.*', 'articulo.*', 'articulotipoventa.*','articulo.estatus as ESTATUSARTICULO')
        ->where('articulotipoventa.fkTipoVenta', 4) 
        ->where('articulo.estatus', '!=', 3)
        ...
        return view($vista,compact('datosArticulos','datosArticulosGenerales','datosEngancheYCosto1','datosEngancheYCosto2','datosEngancheYCosto3','datosEngancheYCosto4'));
    }
...
public function obtenerCantidadTipoVenta(Request  $req,  $id, $tipoVentaSeleccionado)
{
   ...
   // Utilizamos first() para obtener solo un resultado
    // Verifica si se encontró el artículo
    if ($articulo) {
        return response()->json([
            'nombreArticulo' => $articulo->nombreArticulo,
  ...
  public function agregarArticulo(Request $req)
  {
     ...
     if ($req->hasFile('imagenArticulo')) {
      ...
      $arrayEngancheTipoVenta = [0, $req->EngancheUnAño,$req->EngancheUnAño ,$req->EngancheUnAño];
      for ($i = 0; $i < count($arrayTipoVenta); $i++) {
        ...
        $movimientosArticulos->fkArticulo=$articulo->pkArticulo;
        $movimientosArticulos->fkTipoMovimiento=4;
  ...
  public function movimientosArticulo(Request $req)
  {
    date_default_timezone_set('America/Mazatlan');
      ...
      $subarticulos = $req->input('subarticulo');


      if($movimientosArticulos->fkTipoMovimiento==5){

      $cambiosArticulos= new CambiosArticulos();
      $cambiosArticulos->fkMovimientosArticulos=$movimientosArticulos->pkMovimientosArticulos;
      $cambiosArticulos->fkArticulo=  $subarticulos[0];
      $cambiosArticulos->cantidad=$req->cantidadSeleccionada;
      $cambiosArticulos->save();
      $articulito= Articulo::find( $subarticulos[0]);  
      $articulito->cantidadActual=$cantidadPresente+$req->cantidadSeleccionada;
      $articulito->save();
      
      $movement= new MovimientosArticulos();
      $movement->fkArticulo= $subarticulos[0];
      $movement->fkTipoMovimiento=1;
      $movement->cantidad=$req->cantidadSeleccionada;
      $movement->fecha= Carbon::now();
      $movement->fkEmpleado=session('id');
      $movement->save();
      }
  ...
  public function todosmovimientosArticulos(Request $request) {
    ...
    $datosMovimientos = Articulo::join('categoriaarticulo', 'articulo.fkCategoriaArticulo', '=', 'categoriaarticulo.pkCategoriaArticulo')
        ...
        ->get();
        $datosArticle = Articulo::join('categoriaarticulo', 'articulo.fkCategoriaArticulo', '=', 'categoriaarticulo.pkCategoriaArticulo')
        ->join('movimientosarticulos', 'articulo.pkArticulo', '=', 'movimientosarticulos.fkArticulo')
        ->join('tipomovimiento', 'tipomovimiento.pktipoMovimiento', '=', 'movimientosarticulos.fkTipoMovimiento')
        ->join('empleado', 'empleado.pkEmpleado', '=', 'movimientosarticulos.fkEmpleado')
        ->join('cambiosarticulos', 'cambiosarticulos.fkMovimientosArticulos', '=', 'movimientosarticulos.pkMovimientosArticulos')
        ->select('articulo.*')
        ->get();
;
    
        
    return view('listaMovimientos',compact('datosMovimientos','datosArticle'));
}
