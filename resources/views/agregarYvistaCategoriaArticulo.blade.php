        ...
        <div class="p-4">
           ...
           <div class="flex justify-start">
			...
			<div class="flex justify-center">
				<h1 class="text-center font-bold md:text-[2rem]">Agregar Categoria</h1>
			</div>
			<form  action="{{ route('categoriaArticulo.insertar') }}" method="post" class="grid md:grid-cols-3 grid-cols-1 gap-6 p-5">
            @csrf    
            <div>
                    <label for="" class="block mb-2 text-sm font-medium text-gray-900">Nombre Categoria</label>
                    <input  name="nombreCategoriaArticulo" type="text" id="" class="bg-green-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-400 focus:border-green-400 block w-full p-2.5" placeholder="" required>
                </div>
		   ...
		   <div class="bg-white rounded-lg shadow-lg mt-10">
            <div class="flex justify-center">
				<h1 class="text-center font-bold md:text-[2rem] mt-5">  Categoria articulos registrados</h1>
			</div>
			...
			<table id="tabla-categoriasArticulos" class="w-full table-auto mt-[1rem]" id="miTabla" class="display nowrap" width="90%">
				<thead class="text-center">
					<tr class="h-24 text-center">
                                <th class="linea-izquierda border-gray-300">Nombre de Categoria Articulo</th>
                                <th class="linea-izquierda border-gray-300">Opciones</th>
					</tr>
					<tbody>
                    @foreach ($datosArticulo as $dato)
						<tr class="h-20">
                            <td class="linea-izquierda border-gray-300">{{ $dato->nombreCategoriaArticulo }}</td>
							<td class="items-center flex justify-center">
                                <div class="flex justify-between">
                                    <a href="{{route('categoriaArticulo.mostrarPorId', $dato->pkCategoriaArticulo)}}" class="flex mr-5 w-11 h-11 items-center mt-2 bg-green-500 p-2 text-base font-medium text-white rounded-lg hover:bg-green-400">
                                    ...
                                    <form action="{{ route('categoriaArticulo.baja', ['pkArticulo' => $dato->pkCategoriaArticulo]) }}" id="bajaForm" method="POST">
                                        @csrf <!-- Agrega el token CSRF para protección contra falsificación de solicitudes -->
                                        <button  onclick="confirmarBaja(event)"  type="submit" class="flex ml-10 md:ml-5 w-11 h-11 items-center mt-2 bg-green-500 p-2 text-base font-medium text-white rounded-lg hover:bg-green-400">
   ...
   <script  src="https://cdn.datatables.net/plug-ins/1.13.7/api/fnMultiFilter.js"></script>
    ...
    <script>
        ...
        $(document).ready(function () {
            tableCategorias= $('#tabla-categoriasArticulos').DataTable({
                "language": {
                "search": "Buscar:",
            ...
            e.preventDefault();
            tableCategorias.page('previous').draw(false);
            });

            // Agrega evento de clic al botón Next
            $('#nextBtn').on('click', function(e) {
            e.preventDefault();
            tableCategorias.page('next').draw(false);
            });

            $('#busqueda').on('keyup', function (e) {
            var filtroBusqueda = $('#busqueda').val();
            tableCategorias.search(filtroBusqueda).draw();
        });
...
function confirmarBaja(event) {
    ...
    Swal.fire({
        title: '¿Seguro?',
        text: '¿Desea dar de baja esta categoria',
        icon: 'warning',
