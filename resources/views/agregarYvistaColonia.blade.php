...
@endif
        ...
        <div class="p-4">
		   ...
		   <div class="bg-white rounded-lg shadow-lg p-4">
            ...
            <div>
                ...
                <input  name="nombreColonia"  type="text" id="" class="bg-green-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-400 focus:border-green-400 block w-full p-2.5" placeholder="" required>
            </div>
           ...
           <div class="flex mt-10 md:mr-10 p-5">
                        <a href="/municipioVision" class="flex items-center bg-green-500 p-2 text-base font-medium text-white rounded-lg hover:bg-green-400">
						...
						<tr class="h-20">
							...
							<td class="items-center flex justify-center">
                                <div class="flex justify-between">
                                    <a href="{{route('colonia.mostrarPorId', $dato->pkColonia)}}" class="flex mr-5 w-11 h-11 items-center mt-2 bg-green-500 p-2 text-base font-medium text-white rounded-lg hover:bg-green-400">
                                   ...
                                   <form action="{{ route('colonia.baja', $dato->pkColonia) }}" id="bajaForm-{{ $dato->pkColonia }}" method="POST" class="inline">
                                        @csrf
                                        <button onclick="confirmarBaja(event, '{{ $dato->pkColonia }}')" type="submit" class="flex ml-10 md:ml-5 w-11 h-11 items-center mt-2 bg-green-500 p-2 text-base font-medium text-white rounded-lg hover:bg-green-400">
                                            <svg class="flex-shrink-0 w-7 h-7 text-white transition duration-75 group-hover:text-green-" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
  ...
  <script>
    function confirmarBaja(event, id) {
        ...
        Swal.fire({
            title: '¿Seguro?',
            text: '¿Desea dar de baja esta categoría?',
            icon: 'warning',
