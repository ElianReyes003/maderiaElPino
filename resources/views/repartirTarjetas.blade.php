...
<a href="{{ route('cobrador.Tarjetas', ['pkEmpleado' => $datosUsuario->pkEmpleado, 'vista' => 'seleccionarOrdenTarjetas']) }}" class="baja-usuario mt-4" data-pkCompra="">
                  ordena la reparticion
</a>
<a href="{{ route('cobrador.FormAbono', ['pkEmpleado' => $datosUsuario->pkEmpleado, 'vista' => 'eliminarAsignacionTarjetas']) }}" class="baja-usuario mt-4" data-pkCompra="">
                  quitar tarjetas
</a>
    ...
    <div class="p-4 sm:ml-64 mt-16 md:mt-10">
        ...
        <div class="p-4">
		   ...
		   <div class="bg-white rounded-lg shadow-lg p-4">
            ...
            <form  id="formulario"  action="{{ route('reparto.Insertar') }}" enctype="multipart/form-data"  method="post" >
            @csrf
         @php
			...
			<div class="mt-10 grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
				...
				<label for="underline_select" class="sr-only">Calle</label>
				<select name="fkCalle" id="fkCalle" class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none focus:outline-none focus:ring-0 focus:border-green-400 peer">
                    <option value=""></option>
				</select>
				<div class="flex mt-3">
                ...
                <div class="flex justify-center	mt-16">
					<div class="md:p-10 p-5">
						  <div class="flex">
							...
							<!-- Next Button -->
							<button id="nextBt2"  class="flex items-center justify-center px-4 h-10 md:px-10 md:ml-20 ml-10 ms-3 text-base font-medium text-white bg-green-500 border rounded-lg hover:bg-green-400">
							  Siguiente
							</button>
						  </div>
					</div>
    ...
    $('#nextBtn').on('click', function(e) {
      ...
      tableClientes.page('next').draw(false);
        ...
        $('#fkMunicipio, #fkColonia, #fkCalle').change(function () {
            ...
            tableClientes.column(3).search(municipio).draw(); // Filtro para Municipio
            tableClientes.column(4).search(colonia).draw(); // Filtro para Colonia
            tableClientes.column(5).search(calle).draw(); // Filtro para Calle
        });
